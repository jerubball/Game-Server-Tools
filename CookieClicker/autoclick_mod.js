AutoclickMod = {
	...
	// style tags
	float:right; right:56px; z-index:1000000001; z-index:100001; z-index:10001; z-index:500; display:block;
	...
	*/
	...
	delayedInit() {
		...
		this.pantheon = Game.ObjectsById[6].minigame;
		this.grimoire = Game.ObjectsById[7].minigame;
		if (!this.garden || !this.stockMarket || !this.pantheon || !this.grimoire) {
			setTimeout(() => {this.delayedInit();}, 1000);
			console.log(this.name + ' initialize delayed');
		} else {
			...
			this.style.innerHTML = '\n\
				#autoClickModContainer {position:absolute; left:0; z-index:100001; padding:3px;} \n\
	...
	load(str) {
		if (str) {
			this.delayedLoad(str);
		} else {
			console.log(this.name + ' has nothing to load');
		}
	},
	
	delayedLoad(str) {
		if (!this.loaded) {
			setTimeout(() => {this.delayedLoad(str);}, 1000);
			console.log(this.name + ' load delayed');
		} else if (str) {
			...
			if (obj['cookie'] && this.cookieInterval == null) {
				this.cookieToggle();
			}
			if (obj['gold'] && this.goldInterval == null) {
				this.goldToggle();
			}
			if (obj['reindeer'] && this.reindeerInterval == null) {
				this.reindeerToggle();
			}
			if (obj['wrinkler'] && this.wrinklerInterval == null) {
				this.wrinklerToggle();
			}
			if (obj['lump'] && this.lumpInterval == null) {
				this.lumpToggle();
			}
			if (obj['fortune'] && this.fortuneInterval == null) {
				this.fortuneToggle();
			}
			if (obj['garden'] && this.gardenInterval == null) {
				this.gardenToggle();
			}
			if (obj['stock'] && this.stockInterval == null) {
				this.stockToggle();
			}
			if (obj['grimoire'] && this.grimoireInterval == null) {
				this.grimoireToggle();
			}
			this.updateAll();
	...
	gardenToggle(event) {
		...
		} else if (this.loaded && this.garden) {
			this.gardenClick();
			this.gardenInterval = setInterval(() => {this.gardenClick();}, this.gardenTime);
			this.gardenBtn.classList.remove('autoClickModError');
		} else {
			console.log(this.name + ': error: minigame not loaded', this.loaded, this.garden);
			this.gardenBtn.classList.add('autoClickModError');
		}
		this.updateElement(this.gardenBtn, this.gardenInterval);
	},
	gardenClick() {
		if (Game.ObjectsById[2].amount > 0) {
	...
	gardenPlant(plantId, x, y, bound) {
		// check neighbors.
		if (Game.ObjectsById[2].amount > 0 && this.garden.plot[y][x][0] === 0 && this.garden.plot[y][x][1] === 0
	...
	stockToggle(event) {
		...
		} else if (this.loaded && this.stockMarket) {
			this.stockClick();
			this.stockInterval = setInterval(() => {this.stockClick();}, this.stockTime);
			this.stockBtn.classList.remove('autoClickModError');
		} else {
			console.log(this.name + ': error: minigame not loaded', this.loaded, this.stockMarket);
			this.stockBtn.classList.add('autoClickModError');
		}
		this.updateElement(this.stockBtn, this.stockInterval);
	},
	stockClick() {
	...
	grimoireToggle(event) {
		...
		} else if (this.loaded && this.grimoire) {
			this.grimoireClick();
			this.grimoireInterval = setInterval(() => {this.grimoireClick();}, this.grimoireTime);
			this.grimoireBtn.classList.remove('autoClickModError');
		} else {
			console.log(this.name + ': error: minigame not loaded', this.loaded, this.grimoire);
			this.grimoireBtn.classList.add('autoClickModError');
		}
		this.updateElement(this.grimoireBtn, this.grimoireInterval);
	},
	grimoireClick() {
		if (Game.ObjectsById[7].amount > 0 && this.grimoire.magic === this.grimoire.magicM) {
			this.grimoire.castSpell(this.grimoire.spellsById[this.grimoireTarget]);
		}
	},
	grimoireOption(event, state) {
		if (this.loaded && this.grimoire) {
			this.grimoireSetLabel.classList.remove('autoClickModError');
			if (state !== undefined) {
			...
			console.log(this.name + ': error: minigame not loaded', this.loaded, this.grimoire);
			this.grimoireSetLabel.classList.add('autoClickModError');
		}
	},
};

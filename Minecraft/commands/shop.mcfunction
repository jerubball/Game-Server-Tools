...
#@ conditional
give @p[scores={Transaction=1}] paper{HideFlags:1,display:{Name:"{\"text\":\"Money Note\"}",Lore:["{\"text\":\"$100 bill\"}","{\"text\":\"Value does not change\"}","{\"text\":\"Can be renamed\"}","{\"text\":\"Void if disenchanted\"}"]},Enchantments:[{id:binding_curse,lvl:6}]}
#@ conditional
tellraw @p[scores={Transaction=1}] [{"text":"Your new balance is $","color":"yellow"},{"score":{"name":"*","objective":"Wallet"}}]
#@ conditional
scoreboard players set @p[scores={Transaction=1}] Transaction 0

...
if [[ $server == 0 ]]
then
    message="Minecraft Vanilla Server is running!"
else
    message="Minecraft Vanilla Server is **not** running!"
fi

if [[ $ngrok == 0 ]]
then
    link="$(curl http://192.168.163.219:4040/api/tunnels/minecraft | jq -r '.public_url')"
...
if [[ $server == 0 ]]
then
    status="$(mcstatus 192.168.163.219:25565 status)"
